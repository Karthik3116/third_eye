...
// 4) View‑Code endpoints
// 4a) Check (and upsert) device record, report if viewCode exists
...
// 4b) Create or Validate viewCode
app.post('/viewcode/:device', async (req, res) => {
  ...
  if (create) {
    if (doc.viewCode) {
      return res.status(400).json({ error: 'View code already set' });
    }
...
async function requireAuthAndViewCode(req, res, next) {
  ...
  if (!doc.viewCode) {
    return res.status(403).json({ error: 'No view code set' });
  }
