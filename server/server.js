...
if (!MONGO || !ADMIN_API_KEY || !FRONTEND_URL) {
  console.error('❌ MONGO_URI, ADMIN_API_KEY, or FRONTEND_URL not set in .env');
  process.exit(1);
}
...
// ─── 3) SUBSCRIPTION AUTO‑REVOKE (Unchanged) ──────────────────────────────────
const subscriptionTimeouts = {};
...
// ─── 4) SECURITY MIDDLEWARE (NEW SECTION) ────────────────────────────────────

// 4a) Set security HTTP headers
app.use(helmet());

// 4b) Configure CORS to only allow your frontend
app.use(cors({
    origin: FRONTEND_URL,
...
// 4d) Sanitize data against NoSQL query injection
app.use(mongoSanitize());

// 4e) Prevent HTTP Parameter Pollution
app.use(hpp());

// 4f) Rate Limiting to prevent brute-force attacks
...
app.use('/api/auth/login', authLimiter); // Apply stricter limiter to login route

// 4g) Admin Authentication Middleware
...
// ─── 5) BACKGROUND DATA ENDPOINT ────────────────────────────────────────────
// Note: This endpoint is intentionally open to receive data from devices.
// Protection here would depend on the device's capability to send a secret.
app.post('/background_api/:device', async (req, res) => {
  const device      = req.params.device;
  const payload     = req.body;
  const receivedAt  = new Date();

  await LogEntry.create({ deviceId: device, data: payload, receivedAt: receivedAt });
  await RecentData.findOneAndUpdate(
    { deviceId: device },
    { payload: payload, receivedAt: receivedAt },
    { upsert: true, new: true }
...
// ─── 6) FETCH RECENT DATA (CHANGED) ──────────────────────────────────────────
app.get('/recent_background_api_data/:device', async (req, res) => {
  const device = req.params.device;

  // SECURED: The "professor" bypass now requires the admin key
  if (device === 'professor') {
    if (req.header('X-Admin-API-Key') !== ADMIN_API_KEY) {
        return res.status(401).json({ error: 'Unauthorized: Admin access required.' });
    }
    const allRecentData = await RecentData.find({}).lean();
    const storeObject = allRecentData.reduce((acc, doc) => {
        acc[doc.deviceId] = { data: doc.payload, received_at: doc.receivedAt };
        return acc;
    }, {});
    return res.json(storeObject);
  }
  ...
  res.json({ [device]: { data: entryDoc.payload, received_at: entryDoc.receivedAt } });
});


// ... Sections 7-10 (Control, Auth, Feedback) are largely unchanged but are now protected by the global middleware ...


// ─── 11) ADMIN: LIST & GRANT/REVOKE (SECURED) ────────────────────────────────
// SECURED: Both admin endpoints are now protected by the verifyAdminKey middleware.
app.get('/admin/devices', verifyAdminKey, async (_req, res) => {
  await Subscription.deleteMany({ expiresAt: { $lte: new Date() } });
...
app.post('/admin/authorize/:device', verifyAdminKey, async (req, res) => {
  const deviceId = req.params.device;
  const { authorize, hours = 0, minutes = 0 } = req.body;
...
// ... The rest of the file (auth endpoints, etc.) is included for completeness ...
