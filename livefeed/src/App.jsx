...
export default function App() {
  const [step, setStep]               = useState('enterDevice');
  const [deviceId, setDeviceId]       = useState('');
  const [viewCodeInput, setViewCodeInput] = useState('');
  const [accessKey, setAccessKey]     = useState(null);
  const [viewCode, setViewCode]       = useState(null);

  // check if device exists & has viewCode
  ...
  if (accessKey && (accessKey === 'admin' || viewCode)) {
    return (
      <>
        ...
        <Routes>
          <Route index element={
            <Navigate to={accessKey === 'admin' ? '/admin' : '/monitor'} replace/>
          }/>
          <Route path="/monitor" element={
            <UserMonitor accessKey={accessKey} viewCode={viewCode} />
          }/>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 flex items-center justify-center p-4">
      <div className="bg-slate-800/50 backdrop-blur-lg p-6 rounded-2xl shadow-2xl w-full max-w-sm border border-slate-700">
        ...
        {step === 'enterViewCode' && <>
          <p className="text-slate-400 text-sm mb-2">
            Enter view‑code for <code>{deviceId}</code>:
          </p>
          <input
            value={viewCodeInput}
            onChange={e => setViewCodeInput(e.target.value)}
