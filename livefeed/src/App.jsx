...
export default function App() {
  ...
  return (
    <>
      <nav className="p-4 bg-slate-800 text-white flex items-center">
        <Link to="/monitor" className="mr-4 hover:underline">Monitor</Link>
        {deviceId === 'admin' && (
      ...
      <Routes>
        ...
        <Route
          path="/monitor"
          element={<UserMonitor deviceId={deviceId} viewCode={viewCode} />}
        />
        <Route
          path="/admin"
          element={
            deviceId === 'admin'
              ? <AdminPage deviceId={deviceId} viewCode={viewCode} />
              : <Navigate to="/monitor" replace />
          }
        />
        <Route path="*" element={<Navigate to="/" replace />} />
